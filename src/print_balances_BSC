...
async function getTokenBalancesAtBlock(addresses, blockHeight, provider) {
  ...
  const contracts = {
    USDT: createContractInstance(config.tokensBSC.USDT, provider),
    BUSD: createContractInstance(config.tokensBSC.BUSD, provider),
    ALPACA: createContractInstance(config.tokensBSC.ALPACA, provider),
  };
  ...
  for (const address of addresses) {
    ...
    tokenBalances[address] = balances.reduce((acc, curr, index) => {
      if (index < balances.length - 1) {
        ...
        acc.BNB = formatEtherBalance(curr);
      }
      return acc;
    }, {});
...
async function main() {
  ...
  fs.createReadStream('data/addresses.csv')
    ...
    .on('end', async () => {
      ...
      const csvData = ['address,USDT,BNB,BUSD,ALPACA'].concat(
        Object.entries(tokenBalances).map(([address, balances]) =>
          `${address},${balances.USDT},${balances.BNB},${balances.BUSD},${balances.ALPACA}`)
      );

      fs.writeFileSync('data/BSC_Mainnet_Balances.csv', csvData.join('\n'));
      console.log('The balances have been written to BSC_Mainnet_Balances.csv');
    });
}
